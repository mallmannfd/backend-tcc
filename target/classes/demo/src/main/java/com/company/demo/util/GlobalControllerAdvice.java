...
public class GlobalControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<Object> unknownException(HttpClientErrorException ex, WebRequest request) {
        var message = ex.getResponseBodyAsString();
        var stacktrace = Arrays.toString(ex.getStackTrace());
        LOG.warn(message);
        LOG.warn(stacktrace);
        return this.handleExceptionInternal(ex, message, new HttpHeaders(), ex.getStatusCode(), request);
    }
    ...
    private final ResponseEntity<Object> handleCustomException(String message, Exception ex, WebRequest request) {
        ...
        LOG.warn(message);
        LOG.warn(stacktrace);
        return this.handleExceptionInternal(ex, ex, new HttpHeaders(), HttpStatus.EXPECTATION_FAILED, request);
    }
